...
def compute_mi_rmse(mi_true_list, mi_est_list):
    """计算互信息矩阵的 RMSE（展平后计算所有元素的均方根误差）"""
    rmses = []
    for mi_t, mi_e in zip(mi_true_list, mi_est_list):
        diff = mi_t - mi_e  # 3×3 矩阵差
        rmse = np.sqrt(np.mean(diff**2))  # 所有元素的 RMSE
        rmses.append(rmse)
    return np.mean(rmses), np.std(rmses) / np.sqrt(len(rmses))
...
results = {
    ...
    # Mutual Information RMSE
    'model_mi_rmse': [],
    'maxlike_mi_rmse': [],
    'MLME_mi_rmse': [],

    # Mutual Information RMSE error
    'model_mi_rmse_err': [],
    'maxlike_mi_rmse_err': [],
    'MLME_mi_rmse_err': []
}
...
for nProj in nProj_list:
    ...
    # 计算所有互信息矩阵
    mi_true = [mutual_information_matrix(rho) for rho in rho_true]
    mi_model = [mutual_information_matrix(rho) for rho in rho_model]
    mi_maxlike = [mutual_information_matrix(rho) for rho in rho_maxlike]
    mi_MLME = [mutual_information_matrix(rho) for rho in rho_MLME]

    # 计算互信息 RMSE 及其误差条
    rmse_mi_model, err_mi_model = compute_mi_rmse(mi_true, mi_model)
    rmse_mi_maxlike, err_mi_maxlike = compute_mi_rmse(mi_true, mi_maxlike)
    rmse_mi_MLME, err_mi_MLME = compute_mi_rmse(mi_true, mi_MLME)
    ...
    results['model_mi_rmse'].append(rmse_mi_model)
    results['maxlike_mi_rmse'].append(rmse_mi_maxlike)
    results['MLME_mi_rmse'].append(rmse_mi_MLME)
    results['model_mi_rmse_err'].append(err_mi_model)
    results['maxlike_mi_rmse_err'].append(err_mi_maxlike)
    results['MLME_mi_rmse_err'].append(err_mi_MLME)
    ...
    print(f" Model MI RMSE: {rmse_mi_model:.6f} ± {err_mi_model:.6f}")
    print(f" Maxlike MI RMSE: {rmse_mi_maxlike:.6f} ± {err_mi_maxlike:.6f}")
    print(f" MLME MI RMSE: {rmse_mi_MLME:.6f} ± {err_mi_MLME:.6f}")
...
# 保存到 CSV（文件名保持一致，但会包含新列）
df_results.to_csv('3qubit_test_F_P_MI.csv', index=False)
print("\nResults saved to '3qubit_test_F_P_MI.csv'")